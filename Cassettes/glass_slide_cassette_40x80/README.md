# Glass-slide small-parts cassette — prototype v0.8 (Optimized Height, Internal Ridges & Reinforced Clip)

Version 0.8 implements the vertical height optimization (Plan 002), removable divider system (Plan 003), internal flanking wall ridges with reinforced closure clasp (Plan 010), and $1.20\text{ mm}$ reinforced compliant glass retention clip.
...
2. **Reinforced $1.20\text{ mm}$ Compliant Glass Clip:** The compliant tongue and solid roof frame are increased to $1.20\text{ mm}$ (6 solid layers in PETG, +125% stronger) with widened $2.5\text{ mm}$ 3D root gussets. This eliminates bed-peel root tear failures upon removing prints from PEI build plates while maintaining smooth manual slide service.
3. **Internal Flanking Ridges (Plan 010):** Vertical ridges projecting $+0.80\text{ mm}$ into the cavity ($1.50\text{ mm}$ wide along Y) flank the divider slots on the front wall. They create deep $1.40\text{–}1.60\text{ mm}$ guide channels ($3.00\text{ mm}$ total engagement across the cavity) and brace the front wall span down from $80\text{ mm}$ to $22.6\text{ mm}$ around the center latch.
4. **Loose-Fit Gravity Drop-In Dividers:** $33.00\text{ mm}$ wide divider cards provide **$+1.10\text{ mm}$ of lateral float clearance** across the $34.10\text{ mm}$ channel bottom span, completely eliminating outward wall wedging while maintaining $\ge 0.75\text{ mm}$ of positive capture overlap past the channel lips.
5. **Reinforced $0.85\text{ mm}$ Closure Clasp:** $1.25\text{ mm}$ thick cantilever beam on the lid with $0.85\text{ mm}$ undercut catch on the front body wall guarantees firm, positive snap retention in both divided and undivided configurations.
6. **Backwards Compatibility:** Body split-line rim ($Z = 32.80\text{ mm}$), hinge alignment axis, and closure catches are 100% plug-and-play compatible with existing printed v0.8 bodies.
...
- `build/cassette_lid_v0_8_print.stl` (lid with $1.20\text{ mm}$ reinforced compliant clip; print top/label-face down in PETG)
...
The full closed envelope is **39.55 × 80.0 × 36.4 mm**.
...
6. Place cassette inside a 3 × 4 carrier tray and stack a second carrier tray on top to confirm full seating without contact.
