...
async def seed_database_from_json():
    ...
    async with AsyncSessionLocal() as session:
        ...
        cats_res = await session.execute(select(CategoryRecord))
        categories_seeded = cats_res.scalars().first() is not None
        ...
        # Check if BinCompartmentRecord has data
        comp_res = await session.execute(select(BinCompartmentRecord))
        ...
        if not has_compartments:
            ...
            all_parts = [p.id for p in parts_res.scalars().all()]
            ...
            for bin_num in range(1, 25):
                ...
                for c_idx in range(1, comp_count + 1):
                    assigned_part = all_parts[part_ptr % total_parts] if total_parts > 0 else None
                    part_ptr += 1
