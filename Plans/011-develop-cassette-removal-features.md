...
- Provide a robust anchor mechanism:
  ...
  - Add a clean clearance notch in the lid's front perimeter skirt ($Y \in [15.50, 27.50\text{ mm}]$) leaving $\ge 1.5\text{ mm}$ solid plastic between the glass channel and the cutout.
...
2. [x] **Body Dovetail Keyway Integration:** Added $15.0\text{ mm}$ vertical dovetail keyway into the front wall of `build_divided_body()` and `build_body()`.
3. [x] **Lid Clearance Cutaway & Solid Roof Restoration:** Added matching clearance notch to front skirt in `build_lid_local()` and restored solid flat rear roof for label.
4. [x] **Pull Tab Generator (`pull_tab_v0_8.stl`):** Modeled vertical shank male dovetail and ergonomic grip head in `build_pull_tab()`.
5. [x] **Export & Mesh Audit:** Generated and audited all STLs for 0 boundary edges and 0 non-manifold edges.
6. [x] **CAD Multi-Views & Documentation:** Rendered updated multi-views and updated release README.
7. [ ] **Physical Validation:** Print test body, lid, and pull tab in PETG; verify slide fit, lock, lid clearance, and vertical extraction from packed carrier.
