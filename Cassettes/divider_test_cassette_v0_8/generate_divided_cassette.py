...
# Usable internal cavity
CAVITY_W = BODY_W - 2 * BODY_WALL  # 34.60 mm
CAVITY_D = BODY_D - 2 * BODY_WALL  # 76.00 mm
...
FLOOR_GROOVE_D = 0.60
OV = 0.05
SLOT_STATIONS = [0.00, -12.87, 12.87]  # Center (1-divider) and Thirds (2-divider)
...
def build_divided_body() -> Mesh:
    ...
    # 2. Outer left wall: X in [-hx, -ix - SLOT_RECESS + OV], Z in [CAVITY_FLOOR_Z - FLOOR_GROOVE_D, BODY_H]
    m.add(box(-hx, -ix - SLOT_RECESS + OV, -hy + c - OV, hy - c + OV, CAVITY_FLOOR_Z - FLOOR_GROOVE_D, HINGE_BODY_RELIEF_TOP + OV))
    # Upper left sections:
    m.add(box(-hx, -ix - SLOT_RECESS + OV, HINGE_RELIEF_Y0, HINGE_RELIEF_Y1, HINGE_BODY_RELIEF_TOP, HINGE_BODY_SUPPORT_TOP))
    m.add(box(-hx, -ix - SLOT_RECESS + OV, -hy + c - OV, HINGE_BODY_END_RELIEF_Y0, HINGE_BODY_RELIEF_TOP, BODY_H))
    m.add(box(-hx, -ix - SLOT_RECESS + OV, HINGE_BODY_END_RELIEF_Y1, hy - c + OV, HINGE_BODY_RELIEF_TOP, BODY_H))
    
    # 3. Outer right wall: X in [ix + SLOT_RECESS - OV, hx], Z in [CAVITY_FLOOR_Z - FLOOR_GROOVE_D, BODY_H]
    m.add(box(ix + SLOT_RECESS - OV, hx, -hy + c - OV, hy - c + OV, CAVITY_FLOOR_Z - FLOOR_GROOVE_D, BODY_H))
    ...
    for idx in range(0, len(y_points) - 1, 2):
        ...
        m.add(box(-ix - OV, ix + OV, y0 - OV, y1 + OV, CAVITY_FLOOR_Z - FLOOR_GROOVE_D, CAVITY_FLOOR_Z + OV))
        
        # Inner left wall segment between slots:
        m.add(box(-ix - SLOT_RECESS - OV, -ix, y0 - OV, y1 + OV, CAVITY_FLOOR_Z - FLOOR_GROOVE_D, HINGE_BODY_RELIEF_TOP + OV))
        if y1 <= HINGE_BODY_END_RELIEF_Y0 or y0 >= HINGE_BODY_END_RELIEF_Y1:
            m.add(box(-ix - SLOT_RECESS - OV, -ix, y0 - OV, y1 + OV, HINGE_BODY_RELIEF_TOP, BODY_H))
        elif y0 >= HINGE_RELIEF_Y0 and y1 <= HINGE_RELIEF_Y1:
            m.add(box(-ix - SLOT_RECESS - OV, -ix, y0 - OV, y1 + OV, HINGE_BODY_RELIEF_TOP, HINGE_BODY_SUPPORT_TOP))
            
        # Inner right wall segment between slots (Z up to BODY_H):
        m.add(box(ix, ix + SLOT_RECESS + OV, y0 - OV, y1 + OV, CAVITY_FLOOR_Z - FLOOR_GROOVE_D, BODY_H))
    ...
    # 9. Closure catch on inner right wall
    catch_profile = [
        (17.30, BODY_H - 2.50),
        (16.55, BODY_H - 2.08),
        (16.55, BODY_H - 1.72),
