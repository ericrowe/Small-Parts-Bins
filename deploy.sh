...
if [[ -z "$DRY_RUN" ]]; then
  ...
  ssh -o ConnectTimeout=5 -o BatchMode=yes "$PI_USER@$PI_HOST" "sudo mkdir -p $TARGET_DIR /srv/database/parts && sudo chown -R $PI_USER:www-data $TARGET_DIR /srv/database/parts && sudo chmod 775 /srv/database/parts"
fi
